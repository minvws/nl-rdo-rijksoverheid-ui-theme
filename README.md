...
To use the theme, logo and fonts you need permissions from the Government
Information Service(RVD). See also
https://www.rijkshuisstijl.nl/over-de-rijkshuisstijl/auteursrecht-rijkshuisstijl.
...
To use this package you need to use something like
[Sass](https://sass-lang.com/), [webpack](https://webpack.js.org/),
[Laravel Mix](https://laravel-mix.com/) or [Vite](https://vitejs.dev/).

You can find an example webpack configuration file in the
[`html/` folder](html/) of this package.

## Installation

This project is published as an npm package on the GitHub Packages registry. See
...
To authenticate for GitHub packages you need to add a personal access token
(classic) to your user `.npmrc` file with the `read:packages` scope. You can
[generate a new access token (`read:packages`)](https://github.com/settings/tokens/new?scopes=read:packages&description=GitHub+Packages+token)
or read more about
...
[`scss/manon/_index.scss`](scss/manon/_index.scss) for an example of how to
import components from Manon.

After importing your chosen components from Manon, apply the theme by `@use`ing
this package:
...
The second line imports all of the theme-specific components. These bring their
own CSS. If one or more of these components interfere with other CSS in the
application, it's possible to import only specific components, like so:
...
See [`scss/components/_index.scss`](scss/components/_index.scss) for a full list
of the components.
...
In addition to choosing which Manon components to load, it is also possible to
selectively import individual components from the theme. Please keep in mind
that these might change between releases, so this is not recommended.
...
In some cases you may need to change the basepath. Your project might copy the
...
In cases like this, you can configure the asset URL basepaths via SCSS
variables:

### When [used as a Manon theme](#as-a-manon-theme)

Pass the `$font-path` and `$img-path` (**NB: no `ro-` prefix**) when `@use`ing
the theme:
...
Define the `$ro-font-path` and `$ro-img-path` variables before `@import`ing
...
To migrate from Standalone mode to Theme mode, first make sure you're on the
...
Next, update the way you import the theme. In this example we'll assume your
...
start is to copy [`scss/manon/_index.scss`](scss/manon/_index.scss) from the
theme:
...
To run the visual regression tests, you will need docker and docker compose and
you will need to
...
2. Run `docker login ghcr.io --username <username>`, substituting your GitHub
   username.
3. In the password prompt, enter the personal access token you obtained from
   step 1.
...
This will screenshot all test pages and report any visual differences from the
...
To publish a release, tag a commit with a version number, ideally by drafting a
new release via the
[releases page](https://github.com/minvws/nl-rdo-rijksoverheid-ui-theme/releases)
to add release notes. The "github-publish" CI workflow will take care of
